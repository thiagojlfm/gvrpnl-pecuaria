        ...
        <div key={pageKey} className="page-enter" style={{flex:1,overflowY:'auto',padding:'24px 24px',maxWidth:1200,width:'100%',margin:'0 auto',boxSizing:'border-box'}}>
          ...
          {page==='rebanho'&&user&&<>
            ...
            <div style={{...gs(150),marginBottom:20}}>
              ...
              <Metric T={T} icon="💰" label="Receita est." value={`$${fmt(meusLotes.filter(l=>l.fase==='abatido'&&l.status==='ativo').reduce((s,l)=>s+(mercado?.precos?.abate||0)*l.quantidade,0))}`} color={T.gold}/>
            </div>
          ...
          {/* AJUDA */}
          {page==='ajuda'&&<>
            <SectionTitle T={T} icon="❓" title="Como funciona" sub="Tudo que você precisa saber sobre o sistema de pecuária"/>
            <div style={gs(260)}>
              {[
                {emoji:'🌱',title:'O ciclo em 4 semanas',text:'Bezerro → Garrote → Boi → Abate. Cada fase dura 7 dias. Só o abate final gera addmoney. As demais fases são negociação entre jogadores.'},
                {emoji:'🛒',title:'Como comprar',text:'Simule a quantidade na aba Comprar, veja o total (bezerro + frete + ração), pague no servidor e cole o comprovante. O admin aprova e o lote aparece no seu Rebanho.'},
                {emoji:'🌾',title:'Ração — o equilíbrio',text:'Mais de 400 cabeças no servidor = ração +20%. Mais de 600 = +50%. Isso reduz a margem de todos e evita superlotação.'},
                {emoji:'🥩',title:'Solicitar abate',text:'Na semana 4 clique em "Solicitar abate". O admin faz o addmoney no servidor e marca como pago. Você recebe uma notificação.'},
                {emoji:'🤝',title:'Venda entre jogadores',text:'A partir do Garrote você pode anunciar e negociar via chat ao vivo. Mais rápido que esperar o abate, mas com margem menor.'},
                {emoji:'🔔',title:'Notificações',text:'O sino 🔔 no topo avisa quando seu lote avança de fase e quando o abate é pago. Fique de olho!'},
              ].map(c=><Card key={c.title} T={T}>
                <div style={{fontSize:36,marginBottom:12}}>{c.emoji}</div>
                <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:16,fontWeight:700,color:T.text,marginBottom:8}}>{c.title}</h3>
                <p style={{fontSize:13,color:T.textDim,lineHeight:1.8}}>{c.text}</p>
              </Card>)}
            </div>
            <Card T={T} hover={false}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:T.text,marginBottom:14}}>Tabela de fases e preços</div>
              <Tbl T={T} headers={['Fase','Peso','Origem','Destino','Ração/cabeça']}
                rows={[
                  ['🐄 Bezerro — Sem. 1','180kg','Gov. NPC ($1.100)','—','21kg'],
                  ['🐄 Garrote — Sem. 2','400kg','Outro jogador','Outro jogador','35kg'],
                  ['🐄 Boi — Sem. 3','540kg','Outro jogador','Outro jogador','56kg'],
                  ['🥩 Boi Abatido — Sem. 4','648kg','—','Frigorífico NPC','—'],
                ].map(r=>r.map((c,i)=><span style={{color:i===0?T.text:T.textDim,fontWeight:i===0?600:400,fontSize:i===0?13:12}}>{c}</span>))}/>
            </Card>
          </>}
