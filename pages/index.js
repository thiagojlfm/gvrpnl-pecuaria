...
function AnimalCard({ fase, mercado }) {
  const imgs = {
    bezerro: 'https://images.unsplash.com/photo-1570042225831-d98fa7577f1e?w=300&q=80',
    garrote: 'https://images.unsplash.com/photo-1546445317-29f4545e9d53?w=300&q=80',
    boi: 'https://images.unsplash.com/photo-1500595046743-cd271d694d30?w=300&q=80',
    abatido: 'https://images.unsplash.com/photo-1558030006-450675393462?w=300&q=80',
  }
  ...
  const origem = { bezerro: 'Gov. NPC — fixo', garrote: 'Livre p2p', boi: 'Livre p2p', abatido: 'Frigorífico NPC' }
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,.4)' }}>
      <div style={{ height: 130, overflow: 'hidden', position: 'relative', background: '#0a0c07' }}>
        <img src={imgs[fase]} alt={FASES[fase]} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(.75) saturate(.85)' }} onError={e => e.target.style.display='none'} />
        <div style={{ position: 'absolute', top: 8, left: 8 }}><Badge type={badgeT[fase]}>Semana {SEMANAS[fase]}</Badge></div>
        {fase === 'abatido' && <div style={{ position: 'absolute', top: 8, right: 8 }}><Badge type="ok">🥩</Badge></div>}
      </div>
...
export default function App() {
  ...
  const [nLote, setNLote] = useState({ jogador_id:'', jogador_nome:'', fazenda:'', quantidade:1, valor_compra:900, data_compra:'', comprovante:'' })
  ...
  async function enviarSolicitacao() {
    ...
    const cot = calcCotacao(compraQt)
  ...
  async function aprovarSolic(id, status) {
    await api('/api/solicitacoes', { method: 'PATCH', body: JSON.stringify({ id, status }) })
    notify(status === 'aprovado' ? '✓ Compra aprovada!' : 'Compra recusada.')
    api('/api/solicitacoes').then(setSolic)
  }

  async function aprovarUser(id, status) {
    await api('/api/admin/usuarios', { method: 'PATCH', body: JSON.stringify({ id, status }) })
    notify(status === 'aprovado' ? '✓ Jogador aprovado!' : 'Recusado.')
    api('/api/admin/usuarios').then(setUsers)
  }
  ...
  const nav = [
    ...
    { id: 'p2p', label: '🤝 P2P', pub: false },
    { id: 'admin', label: '⚙️ Admin', pub: false, admin: true },
    { id: 'hist', label: '📋 Histórico', pub: false },
  ]
      ...
      <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background: C.bg, minHeight: '100vh', color: C.text }}>
        ...
        <div style={{ background: '#0c1408', borderBottom: `1px solid ${C.border}`, padding: '0 24px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg,#1a3a0a,#4a7a1e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🐄</div>
            <div>
        ...
        <div style={{ padding: 24, maxWidth: 1280, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '70vh' }}>
              <div style={{ background: C.card, border: `1px solid ${C.border2}`, borderRadius: 16, padding: 36, width: 360, boxShadow: '0 8px 32px rgba(0,0,0,.5)' }}>
                <div style={{ textAlign: 'center', marginBottom: 28 }}>
                  <div style={{ width: 56, height: 56, borderRadius: 14, background: 'linear-gradient(135deg,#1a3a0a,#4a7a1e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, margin: '0 auto 12px', boxShadow: '0 4px 16px rgba(74,122,30,.4)' }}>🐄</div>
                ...
                {authTab === 'login' ? <>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 18 }}>
                    <Input label="Usuário" value={regForm.username} onChange={e => setRegForm(f => ({...f, username: e.target.value}))} placeholder="nome_no_servidor" />
                    <Input label="Senha" type="password" value={regForm.password} onChange={e => setRegForm(f => ({...f, password: e.target.value}))} placeholder="mínimo 6 caracteres" />
                    <Input label="Fazenda (opcional)" value={regForm.fazenda} onChange={e => setRegForm(f => ({...f, fazenda: e.target.value}))} placeholder="Ex: 0325" />
                  </div>
          ...
          {page === 'mercado' && (
            <>
              <Title t="📈 Mercado — Pecuária" s="Preços dinâmicos baseados no rebanho ativo" />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <Card glow>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    ...
                    <Metric label="Margem estimada" value={`${mercado?.margem||'~35'}%`} sub="bezerro → abate" color={C.green} />
                ...
                <Card>
                  ...
                  <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10, marginTop: 6, display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                    <span style={{ color: C.textMuted }}>Limite: 400 cab.</span>
              ...
              <Card>
                ...
                <Tbl headers={['Entra em','Custo c/ ração','Receita abate','Margem est.']}
                  rows={mercado ? [
          ...
          {page === 'comprar' && user && (
            <>
              <Title t="🛒 Comprar Bezerros" s="Simule, confirme e envie o comprovante — admin aprova" />
              {compraStep === 3 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <Card glow>
                    <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 18 }}>
                      {compraStep===1 ? '1. Simule sua compra' : '2. Pague e envie comprovante'}
                    </div>
                    {compraStep===1 && <>
                      <div style={{ marginBottom: 20 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                          <button onClick={() => setCompraQt(Math.max(1,compraQt-1))} style={{ width: 38, height: 38, borderRadius: 8, background: C.border, border: 'none', color: C.text, fontSize: 20, cursor: 'pointer', fontFamily: 'inherit' }}>−</button>
                          ...
                          <button onClick={() => setCompraQt(compraQt+1)} style={{ width: 38, height: 38, borderRadius: 8, background: C.border, border: 'none', color: C.text, fontSize: 20, cursor: 'pointer', fontFamily: 'inherit' }}>+</button>
                        </div>
                      </div>
                      {cot && <>
                        <div style={{ background: '#0d1009', borderRadius: 10, padding: 16, marginBottom: 16, border: `1px solid ${C.border}` }}>
                          ...
                          {[
                            [`${cot.qty}× Bezerro ($900/cab)`, cot.custoBezerros],
                    ...
                    {compraStep===2 && cot && <>
                      ...
                      <div style={{ background: '#0d1009', borderRadius: 10, padding: 14, marginBottom: 16, border: `1px solid ${C.border}` }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
                          ...
                          <span style={{ fontWeight: 700, color: C.gold }}>${fmt(cot.total)}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                          <span style={{ color: C.textMuted }}>Margem estimada</span>
                          <span style={{ fontWeight: 600, color: C.green }}>{cot.margem}%</span>
                        </div>
                      </div>
                      <div style={{ marginBottom: 16 }}>
                        <Input label="Link do comprovante (Discord)" value={compraComp} onChange={e => setCompraComp(e.target.value)} placeholder="https://discord.com/channels/..." />
                      </div>
                      <div style={{ display: 'flex', gap: 10 }}>
                        <Btn v="ghost" onClick={() => setCompraStep(1)} style={{ flex: 1 }}>Voltar</Btn>
                        <Btn onClick={enviarSolicitacao} style={{ flex: 2, padding: 12 }}>Enviar solicitação</Btn>
                      </div>
                    </>}
                  </Card>
                  <Card>
          ...
          {page === 'rebanho' && user && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 18 }}>
                ...
                <Metric label="Receita est. abate" value={`$${fmt(meusLotes.filter(l=>l.fase==='abatido'&&l.status==='ativo').reduce((s,l)=>s+(mercado?.precos?.abate||0)*l.quantidade,0))}`} />
              </div>
              <Card>
                ...
                <Tbl headers={['Lote','Qtd','Fase','Peso','Pronto em','Consumo/dia','Valor ref.','Status','']}
                  rows={meusLotes.map(l => {
                    ...
                    return [
                      <span style={{fontWeight:700,color:C.text}}>{l.codigo}</span>,
          ...
          {page === 'p2p' && (
            <>
              <Title t="🤝 Venda entre Jogadores" s="Garrotes e bois · preço livre · admin registra" />
              <Alert type="info">Comprar numa fase avançada poupa tempo mas reduz margem.</Alert>
              <Card>
                ...
                <Tbl headers={['Vendedor','Fazenda','Fase','Qtd','Peso','Preço','Obs','']}
                  rows={anuncios.filter(a=>a.status==='ativo').map(a => [
                    ...
                    a.obs||'—',
                    <Btn v="ghost" style={{padding:'4px 10px',fontSize:11}} onClick={()=>notify('Fale com o vendedor no Discord!')}>Interesse</Btn>
                  ])} />
              </Card>
              {user && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <Card>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                      ...
                      <Input label="Preço ($)" type="number" value={nAnuncio.preco_pedido} onChange={e=>setNAnuncio(f=>({...f,preco_pedido:e.target.value}))} placeholder="1800" />
                      <Input label="Obs" value={nAnuncio.obs} onChange={e=>setNAnuncio(f=>({...f,obs:e.target.value}))} placeholder="Negociável..." style={{gridColumn:'1/-1'}} />
                    </div>
                  ...
                  {user?.role==='admin' && (
                    <Card>
                      <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 14 }}>Registrar venda p2p <Badge type="amber">Admin</Badge></div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                        ...
                        <Input label="Comprador" value={p2p.comprador_nome} onChange={e=>setP2p(f=>({...f,comprador_nome:e.target.value}))} placeholder="NomeJogador" />
                        <Input label="Preço final ($)" type="number" value={p2p.preco_final} onChange={e=>setP2p(f=>({...f,preco_final:e.target.value}))} />
                      </div>
                      <Btn onClick={async()=>{const r=await api('/api/anuncios',{method:'PATCH',body:JSON.stringify(p2p)});if(!r.error){notify('✓ Venda p2p registrada!');api('/api/lotes').then(setLotes);api('/api/anuncios').then(setAnuncios);api('/api/transacoes').then(setTrans)}}}>Registrar venda</Btn>
                    </Card>
                  )}
                </div>
              )}
          ...
          {page === 'admin' && user?.role === 'admin' && (
            <>
              <Title t="⚙️ Admin — Pecuária" s="Aprovar compras, confirmar abates, gerenciar jogadores" />
              ...
              <Card>
                ...
                <Tbl headers={['Jogador','Qtd','Total','Ração','Comprovante','','']}
                  rows={solicPend.map(s => [
                    ...
                    <Btn onClick={()=>aprovarSolic(s.id,'aprovado')} style={{padding:'4px 10px',fontSize:11}}>✓ Aprovar</Btn>,
                    <Btn v="danger" onClick={()=>aprovarSolic(s.id,'recusado')} style={{padding:'4px 10px',fontSize:11}}>✗ Recusar</Btn>
                  ])} />
              </Card>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <Card>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                    ...
                    <Input label="Fazenda" value={nLote.fazenda} onChange={e=>setNLote(f=>({...f,fazenda:e.target.value}))} placeholder="0325" />
                    <Input label="Quantidade" type="number" value={nLote.quantidade} onChange={e=>setNLote(f=>({...f,quantidade:Number(e.target.value)}))} />
                    <Input label="Preço/cab ($)" type="number" value={nLote.valor_compra} onChange={e=>setNLote(f=>({...f,valor_compra:Number(e.target.value)}))} />
                    <Input label="Data compra" type="date" value={nLote.data_compra} onChange={e=>setNLote(f=>({...f,data_compra:e.target.value}))} />
                    <Input label="Comprovante" value={nLote.comprovante} onChange={e=>setNLote(f=>({...f,comprovante:e.target.value}))} placeholder="discord.com/..." />
                  </div>
                  <Btn onClick={async()=>{const r=await api('/api/lotes',{method:'POST',body:JSON.stringify(nLote)});if(!r.error){notify('✓ Lote '+r.codigo+' registrado!');api('/api/lotes').then(setLotes);api('/api/transacoes').then(setTrans)}}}>Registrar lote</Btn>
                </Card>
                <Card>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 8 }}>Abates — aguardando addmoney</div>
                  <div style={{ fontSize: 12, color: C.textMuted, marginBottom: 14 }}>Faça o addmoney, depois marque como pago.</div>
                  <Tbl headers={['Jogador','Lote','Qtd','Valor','']}
                    rows={abatesPend.map(l => [
                      ...
                      <Btn v="amber" onClick={async()=>{const r=await api(`/api/lotes/${l.id}`,{method:'PATCH',body:JSON.stringify({action:'marcar_pago'})});if(!r.error){notify('✓ Pago!');api('/api/lotes').then(setLotes);api('/api/transacoes').then(setTrans)}}} style={{padding:'4px 8px',fontSize:11}}>✓ Pago</Btn>
                    ])} />
                </Card>
              </div>
              ...
              <Card>
                ...
                <Tbl headers={['Jogador','Fazenda','Lote','Qtd','Fase','Pronto em','Status','']}
              ...
              <Card>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: 12, alignItems: 'end' }}>
                  ...
                  <Input label="Quantidade (kg)" type="number" value={nRacao.kg} onChange={e=>setNRacao(f=>({...f,kg:e.target.value}))} placeholder="500" />
                  <Input label="Valor pago ($)" type="number" value={nRacao.valor} onChange={e=>setNRacao(f=>({...f,valor:e.target.value}))} placeholder="1000" />
                ...
                {usersPend.length===0
                  ...
                  : <Tbl headers={['Usuário','Fazenda','Solicitado em','','']}
                      rows={usersPend.map(u => [
                        ...
                        <Btn onClick={()=>aprovarUser(u.id,'aprovado')} style={{padding:'4px 10px',fontSize:11}}>✓ Aprovar</Btn>,
                        <Btn v="danger" onClick={()=>aprovarUser(u.id,'recusado')} style={{padding:'4px 10px',fontSize:11}}>✗ Recusar</Btn>
                      ])} />
                }
              </Card>
              ...
              <Card>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: 12, marginBottom: 14, alignItems: 'end' }}>
                  <Input label="Usuário" value={nUser.username} onChange={e=>setNUser(f=>({...f,username:e.target.value}))} placeholder="nome_jogador" />
                  <Input label="Senha" type="password" value={nUser.password} onChange={e=>setNUser(f=>({...f,password:e.target.value}))} placeholder="senha123" />
                  <Input label="Fazenda" value={nUser.fazenda} onChange={e=>setNUser(f=>({...f,fazenda:e.target.value}))} placeholder="0325" />
          ...
          {page === 'hist' && (
            <>
              <Title t="📋 Histórico de Transações" s="Compras NPC, ração, p2p e abates" />
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 18 }}>
                <Metric label="Total transações" value={trans.length} />
                <Metric label="Abates" value={trans.filter(t=>t.tipo==='abate').length} />
                <Metric label="Vendas p2p" value={trans.filter(t=>t.tipo==='p2p').length} />
                <Metric label="Volume total" value={`$${fmt(trans.reduce((s,t)=>s+Number(t.valor),0))}`} color={C.green} />
              </div>
              <Card>
                <Tbl headers={['Data','De','Para','Lote','Qtd','Valor','Tipo']}
                  rows={trans.map(t => [
                    new Date(t.criado_em).toLocaleDateString('pt-BR'),
                    t.de_jogador, t.para_jogador, t.lote_codigo||'—', t.quantidade||'—',
                    <span style={{color:C.green,fontWeight:600}}>${fmt(t.valor)}</span>,
                    <Badge type={t.tipo==='abate'?'ok':t.tipo==='p2p'?'purple':t.tipo==='compra_racao'?'amber':'info'}>
                      {t.tipo==='abate'?'Abate':t.tipo==='p2p'?'P2P':t.tipo==='compra_racao'?'Ração':'NPC'}
                    </Badge>
                  ])} />
              </Card>
            </>
